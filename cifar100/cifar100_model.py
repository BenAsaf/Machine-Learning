...
NUM_CLASSES = cifar100_dataset.NUM_CLASSES
NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN = cifar100_dataset.NUM_IMAGES["train"]
NUM_EXAMPLES_PER_EPOCH_FOR_EVAL = cifar100_dataset.NUM_IMAGES["validation"]
...
def get_model(x, bIsTraining):
	with tf.name_scope("Model"):
		...
		for i in range(4):
			nn = tf.layers.batch_normalization(nn, axis=-1, training=bIsTraining)
			nn = tf.layers.conv2d(nn, filters=512, kernel_size=3, strides=(1, 1), padding="same")
			nn = tf.nn.relu(nn)
