...
def train_net():
	...
	# Making sure CPU takes care of preparing the data for the GPU
	...
	class _LoggerHook(tf.train.SessionRunHook):
		...
		def before_run(self, run_context):
			return tf.train.SessionRunArgs([loss_op, accuracy_op], feed_dict={bIsTraining: True})
	...
	with tf.train.MonitoredTrainingSession(
			...
			config=tf.ConfigProto(log_device_placement=ARGS.log_device_placement),  # Hmm doesnt work, still logs.
			scaffold=scaffold,
...
def test_model():
	...
	logits = get_model(images, bIsTraining)
	# total_correct, accuracy_op = tf.metrics.accuracy(abels, logits)
